...
pub async fn run_find_references(
    ...
    let lsp_result = try_lsp_references(&path, &content, &language, params.line, params.column, state).await;
    let mut references = match lsp_result {
...
async fn try_lsp_references(
    ...
    let task = tokio::task::spawn_blocking(move || {
        ...
        session.request_references(&path_buf, lang_id, &content, line, column)
    });
...
fn fallback_grep(symbol: &str, language: &str, state: &AppState) -> Vec<ReferenceLocation> {
    ...
    match result {
        Ok(r) => r.matches.into_iter().map(|m| ReferenceLocation {
            path: m.path,
            line: m.line,
            column: m.column,
            preview: m.content,
