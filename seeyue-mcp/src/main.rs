...
impl SeeyueMcpServer {
    ...
    #[tool(description = "\
        Delete a named memory entry from .ai/memory/. \
        Removes the content file and the index.json entry. \
        Returns not_found if the key does not exist.")]
    async fn memory_delete(
        &self,
        Parameters(p): Parameters<MemoryDeleteParams>,
    ) -> Result<CallToolResult, ErrorData> {
        tools::memory_delete::run_memory_delete(
            tools::memory_delete::MemoryDeleteParams { key: p.key },
            &self.state.workspace,
        )
        .map(|r| to_text(serde_json::to_string_pretty(&r).unwrap()))
        .map_err(to_mcp_err)
    ...
    #[tool(description = "\
        List all persisted memory entries from .ai/memory/. \
        Supports optional tag filter and limit. \
        Returns entries sorted by updated timestamp descending.")]
    async fn memory_list(
        &self,
        Parameters(p): Parameters<MemoryListParams>,
    ) -> Result<CallToolResult, ErrorData> {
        tools::memory_list::run_memory_list(
            tools::memory_list::MemoryListParams {
                tag:   p.tag,
                limit: p.limit,
            },
            &self.state.workspace,
    ...
    #[tool(description = "\
        ...
        Returns file path, tool name, and captured_at timestamp per snapshot. \
        Use with rewind to undo specific file changes.")]
    async fn checkpoint_list(
        &self,
        Parameters(_): Parameters<CheckpointListParams>,
    ) -> Result<CallToolResult, ErrorData> {
        tools::checkpoint_list::run_checkpoint_list(
            tools::checkpoint_list::CheckpointListParams {},
            &self.state.checkpoint,
        )
        .map(|r| to_text(serde_json::to_string_pretty(&r).unwrap()))
        .map_err(to_mcp_err)
    ...
    #[tool(description = "\
        ...
        Returns per-node TDD progression: red_verified, green_verified, refactor_done. \
        Optionally filter to a specific node_id.")]
    async fn tdd_evidence_summary(
        &self,
        Parameters(p): Parameters<TddEvidenceParams>,
    ) -> Result<CallToolResult, ErrorData> {
        tools::tdd_evidence::run_tdd_evidence(
            tools::tdd_evidence::TddEvidenceParams { node_id: p.node_id },
            &self.state.workflow_dir,
        )
        .map(|r| to_text(serde_json::to_string_pretty(&r).unwrap()))
        .map_err(to_mcp_err)
    ...
    #[tool(description = "\
        [Hook] End session and persist a session summary to .ai/memory/sessions/. \
        Extracts nodes visited and files written from journal for the current run_id. \
        Returns memory_key of the saved entry.")]
    async fn sy_session_end(
        &self,
        Parameters(p): Parameters<SessionEndParams>,
    ) -> Result<CallToolResult, ErrorData> {
        let result = tools::hooks::run_session_end(
            tools::hooks::SessionEndParams { note: p.note },
            &self.state,
        );
        Ok(to_text(serde_json::to_string_pretty(&result).unwrap()))
    }
...
impl ServerHandler for SeeyueMcpServer {
    ...
    async fn read_resource(
        ...
        resources::workflow::read_resource(&request.uri, &self.state.workflow_dir, &self.state.workspace)
