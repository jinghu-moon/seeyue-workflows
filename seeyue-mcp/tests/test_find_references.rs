// tests/test_find_references.rs
//
// Tests for tools::find_references::run_find_references.
// Run: cargo test --test test_find_references
...
use seeyue_mcp::tools::find_references::{FindReferencesParams, run_find_references};
...
#[tokio::test]
async fn test_find_references_file_not_found_errors() {
    let state = make_state(&ws());
    let err = run_find_references(
        FindReferencesParams { path: "src/does_not_exist_xyz.rs".into(), line: 1, column: 1 },
        &state,
    ).await.unwrap_err();
    let msg = format!("{err:?}");
...
#[tokio::test]
async fn test_find_references_path_escape_blocked() {
    let state = make_state(&ws());
    let err = run_find_references(
        FindReferencesParams { path: "../../outside.rs".into(), line: 1, column: 1 },
        &state,
    ).await.unwrap_err();
    let msg = format!("{err:?}");
...
#[tokio::test]
async fn test_find_references_returns_result_struct() {
    let state = make_state(&ws());
    let result = run_find_references(
        FindReferencesParams { path: "src/main.rs".into(), line: 10, column: 5 },
        &state,
    ).await.unwrap();
...
#[tokio::test]
async fn test_find_references_references_accessible() {
    let state = make_state(&ws());
    let result = run_find_references(
        FindReferencesParams { path: "src/main.rs".into(), line: 10, column: 5 },
        &state,
...
#[tokio::test]
async fn test_find_references_total_matches_len() {
    let state = make_state(&ws());
    let result = run_find_references(
        FindReferencesParams { path: "src/main.rs".into(), line: 10, column: 5 },
        &state,
    ).await.unwrap();
    assert_eq!(result.total, result.references.len());
}
