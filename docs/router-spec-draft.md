...
- `condition=false`：node 不进入当前 run 的 required execution set，并应记录 `node_bypassed` 类事件

补充说明：

- 当前 runtime 尚未定义 `skipped` / `not_applicable` 状态
- 因此 V1 可以先用事件与 route basis 表达 bypass 语义
- 后续若 runtime 引入显式状态，再做进一步收敛
...
- `journal` 事件中补 `phase_completed`
- `journal` 事件中补 `node_bypassed`
- `task_graph.nodes` 增加 `capability`
- `task_graph.nodes` 增加 `priority`
- `task_graph.nodes` 增加最小版 `condition`
