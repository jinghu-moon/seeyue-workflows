...
[RULE]
The runtime SHOULD model three context tiers:
- Hot Context
- Warm Context
- Cold Context

[RULE]
Hot Context SHOULD contain the active capsule id, evidence refs, and recommended_next.

[RULE]
Warm Context SHOULD contain task_id, verdict, and constraints.

[RULE]
Cold Context SHOULD contain journal_ref, checkpoints_dir, and capsules_dir.

[RULE]
Compaction SHOULD be evaluated by context-manager using:
...
[RULE]
Compaction is explicit (not implicit) and requires a context-manager call; the runtime does not auto-compact today.

[RULE]
Compaction SHOULD generate or refresh a capsule and reduce prompt load to the minimum data needed for the next step.
...
[RULE]
L1 SHOULD load only the minimum required skill stubs for the current turn, and defer full skill bodies until invocation.
...
[RULE]
Each persona invocation SHOULD receive a capsule-scoped input package (generated by context-manager).
It SHOULD NOT depend on full transcript replay by default.

### 10.4 L3 Validation

[RULE]
L3 is the machine validation layer.
It MUST verify state integrity, phase legality, and node legality. Report freshness and evidence completeness are enforced by `Stop` gates and verification hooks.
