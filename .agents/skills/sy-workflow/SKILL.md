...
- `.claude/settings.json` defines all hook wiring.
- `seeyue-mcp/target/release/sy-hook.exe` is the unified Rust hook binary.
...
Load `sy-mcp` when any `seeyue-mcp` tool is invoked. Key call points:

| Workflow Stage | MCP Tools |
|---------------|----------|
| Session start | `sy_session_start`, `session_summary` |
| Before any write | `sy_pretool_write(path)` |
| After any write | `sy_posttool_write(path, tool)` |
| Before bash | `sy_pretool_bash(command)` |
| Phase transition | `sy_create_checkpoint(label)` |
| Node state change | `sy_advance_node(node_id, ...)` |
| Before stop | `sy_stop()` |

Session context from verdict responses MUST be checked for `budget_exceeded` and
`restore_pending` before proceeding.
